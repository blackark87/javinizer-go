...
// is available.
func New(cfg *Config, genreProcessor genreProcessorInterface, wordProcessor wordProcessorInterface, aliasResolver aliasResolverInterface, actressLookupRepo ...database.ActressLookupRepositoryInterface) *Aggregator {
	if cfg == nil {
		return nil
	}
	...
	if len(actressLookupRepo) > 0 {
		agg.actressLookupRepo = actressLookupRepo[0]
	}
	agg.resolvePriorities()
	return agg
}

// NewWithDatabase is the compatibility composition root used by focused
// aggregation tests and callers that already own a database handle.
func NewWithDatabase(cfg *config.Config, db *database.DB) *Aggregator {
	aggCfg := ConfigFromAppConfig(cfg)
	if aggCfg == nil {
		return nil
	}
	return New(
		aggCfg,
		NewGenreProcessor(aggCfg.Metadata, database.NewGenreReplacementRepository(db)),
		NewWordProcessor(aggCfg.Metadata, database.NewWordReplacementRepository(db)),
		NewAliasResolver(aggCfg.Metadata, database.NewActressAliasRepository(db)),
		database.NewActressLookupRepository(db),
	)
}
