...
func (a *Aggregator) ApplyConfiguredTranslation(movie *models.Movie) string {
	...
	translatedRecords, warning, err := service.TranslateMovieRecords(ctx, movie, settingsHash)
	if err != nil {
		id := movie.ID
		if id == "" {
			id = movie.ContentID
	...
	for _, translatedRecord := range translatedRecords {
		logging.Debugf("Translation: appending %s translation (title=%q, actresses=%d, hash=%s)", translatedRecord.Language, translatedRecord.Title, len(translatedRecord.Actresses), translatedRecord.SettingsHash)
