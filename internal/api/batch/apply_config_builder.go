...
func resolveOrganizeApplyConfig(
	...
	if req.Resume {
		...
		if err != nil {
			return worker.ApplyPhaseConfig{}, fmt.Errorf("load completed file operations for resume: %w", err)
		}
		applyOpts.SkipFiles = make(map[string]bool, len(operations))
		for _, operation := range operations {
			if operation.NewPath != "" && operation.RevertStatus == models.RevertStatusApplied {
				applyOpts.SkipFiles[operation.OriginalPath] = true
			}
		}
	}
