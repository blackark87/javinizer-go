...
func movieResultToResponse(mr *worker.MovieResult, prov *worker.ProvenanceData, actressRepos ...database.ActressRepositoryInterface) *contracts.BatchFileResult {
	if mr == nil {
		return nil
	}
...
func buildBatchJobResponse(job *worker.BatchJobStatus, actressRepos ...database.ActressRepositoryInterface) *contracts.BatchJobResponse {
	...
	for filePath, fileResult := range job.Results {
		...
		results[filePath] = movieResultToResponse(fileResult, prov, actressRepos...)
	}
