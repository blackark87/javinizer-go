...
func organizeJob(rt *core.APIRuntime) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var job worker.BatchJobInterface
		var err error
		if req.Resume {
			job, err = prepareBatchRequest(snap, c)
			if err == nil {
				status := job.GetJobStatus()
				if status != models.JobStatusCompleted && status != models.JobStatusCancelled {
					c.JSON(http.StatusBadRequest, contracts.ErrorResponse{Error: "Job must be completed or cancelled before resuming organization"})
					return
				}
			}
		} else {
