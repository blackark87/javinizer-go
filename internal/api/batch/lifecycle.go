...
func getBatchJobFull(deps *core.APIDeps, c *gin.Context, jobID string) {
	...
	c.JSON(http.StatusOK, buildBatchJobResponse(job, deps.Repos.ActressRepo))
}
...
func parseAndConvertJobResults(job *models.Job, fs afero.Fs, actressRepos ...database.ActressRepositoryInterface) map[string]*contracts.BatchFileResult {
	if job.Results == "" {
	...
	for filePath, mr := range parsed.Results {
		...
		result := movieResultToResponse(mr, prov, actressRepos...)
		if result.ResultID == "" {
