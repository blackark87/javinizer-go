...
func TestReviewBatchMovieTranslation_RecoversTranslationFailure(t *testing.T) {
	llm := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, err := io.ReadAll(r.Body)
		require.NoError(t, err)
		w.Header().Set("Content-Type", "application/json")
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/batch/"+job.GetID()+"/results/failed-translation/translation-review",
		strings.NewReader(`{"field":"title"}`),
	)
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	...
	var response contracts.TranslationReviewResponse
	require.NoError(t, json.Unmarshal(w.Body.Bytes(), &response))
	assert.True(t, response.Recovered)
	updated := job.GetStatus().Results[filePath]
	...
	assert.Equal(t, 1, job.GetStatus().Completed)
	assert.Zero(t, job.GetStatus().Failed)
}
