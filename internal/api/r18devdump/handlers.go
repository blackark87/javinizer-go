...
const (
	...
	importHeartbeatInterval = 1500 * time.Millisecond
)
...
type dumpHandler struct {
	...
	heartbeatInterval   time.Duration // test override; zero uses importHeartbeatInterval
...
func (h *dumpHandler) runImportHeartbeat(streamConsumed, importDone <-chan struct{}) {
	...
	if interval <= 0 {
		interval = importHeartbeatInterval
	}
