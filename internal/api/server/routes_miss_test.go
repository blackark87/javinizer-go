...
func TestIsSameOrigin_SameHostPort(t *testing.T) {
	...
	result := isSameOriginWithConfig("http://localhost:8080", req, nil)
	assert.True(t, result)
}
...
func TestIsSameOrigin_HttpsDefaultPort(t *testing.T) {
	...
	result := isSameOriginWithConfig("http://example.com", req, nil)
	assert.True(t, result)
}
