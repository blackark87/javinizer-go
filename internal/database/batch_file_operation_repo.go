...
// FindLatestAppliedByMovieID returns the newest non-reverted operation that
// recorded an NFO path for the movie.
func (r *BatchFileOperationRepository) FindLatestAppliedByMovieID(ctx context.Context, movieID string) (*models.BatchFileOperation, error) {
	var op models.BatchFileOperation
	err := r.GetDB().WithContext(ctx).
		Where("movie_id = ? AND revert_status = ? AND TRIM(COALESCE(nfo_path, '')) <> ''", movieID, models.RevertStatusApplied).
