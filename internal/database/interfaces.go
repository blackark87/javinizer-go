...
// ActressLookupRepositoryInterface is the narrow, read-only compatibility
// contract used by the aggregator when it enriches a kanji-only scrape with a
// previously stored romaji/Hangul reading.  It intentionally omits context so
// lightweight scraper/aggregator tests can provide an in-memory lookup.
