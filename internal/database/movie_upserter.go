...
func (u *MovieUpserter) mergeActressData(existing *models.Actress, new models.Actress) bool {
	...
	if new.FirstName != "" && (existing.FirstName == "" ||
		(containsHangulName(new.FirstName) && !containsHangulName(existing.FirstName))) {
		existing.FirstName = new.FirstName
		needsUpdate = true
	}
	if new.LastName != "" && (existing.LastName == "" ||
		(containsHangulName(new.LastName) && !containsHangulName(existing.LastName))) {
...
func containsHangulName(value string) bool {
...
func lookupActressByJapaneseName(tx *gorm.DB, act *models.Actress) (models.Actress, bool, error) {
	var found models.Actress
	err := tx.Where("japanese_name = ?", act.JapaneseName).
		Order("dmm_id DESC, id ASC").First(&found).Error
	if err == nil {
		return found, true, nil
	}
	if !errors.Is(err, gorm.ErrRecordNotFound) {
	...
	key := normalizeExactActressName(act.JapaneseName)
	if key == "" {
		return found, false, nil
	}
	var candidates []models.Actress
	if err := tx.Where("TRIM(COALESCE(japanese_name, '')) <> ''").
		Order("dmm_id DESC, id ASC").Find(&candidates).Error; err != nil {
		return found, false, err
	}
	for _, candidate := range candidates {
		if normalizeExactActressName(candidate.JapaneseName) == key {
...
func lookupActressByName(tx *gorm.DB, act *models.Actress) (models.Actress, bool, error) {
	if act.DMMID != 0 {
		return lookupActressByDMMID(tx, act)
	}
	if act.JapaneseName != "" {
		return lookupActressByJapaneseName(tx, act)
	}

	firstKey := normalizeExactActressName(act.FirstName)
	lastKey := normalizeExactActressName(act.LastName)
	if firstKey == "" && lastKey == "" {
		return models.Actress{}, false, nil
	}
	var candidates []models.Actress
	if err := tx.Order("dmm_id DESC, id ASC").Find(&candidates).Error; err != nil {
		return models.Actress{}, false, err
	}
	for _, candidate := range candidates {
		candidateFirst := normalizeExactActressName(candidate.FirstName)
		candidateLast := normalizeExactActressName(candidate.LastName)
		switch {
		case firstKey != "" && lastKey != "":
			if candidateFirst == firstKey && candidateLast == lastKey {
				return candidate, true, nil
			}
		case firstKey != "":
			if candidateFirst == firstKey {
				return candidate, true, nil
			}
		case lastKey != "":
			if candidateLast == lastKey {
				return candidate, true, nil
			}
		}
	}
