...
type MovieTranslation struct {
	...
	Actresses     []string  `json:"actresses" gorm:"serializer:json"`      // Translated actress names for template rendering
...
func (m *Movie) Clone() *Movie {
	...
	if m.Translations != nil {
		...
		for i := range clone.Translations {
			clone.Translations[i].Actresses = append([]string(nil), m.Translations[i].Actresses...)
		}
	}
	return &clone
}
