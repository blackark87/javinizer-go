...
// DefaultConfig returns a fresh default NFO configuration.
func DefaultConfig() *Config {
...
// MovieToNFO exposes the in-memory NFO representation for focused update
// workflows. Normal file generation should continue to use Generate or
// GenerateAtPath; this method is useful when a caller must preserve unknown XML
// fields and replace only one well-defined section.
func (g *Generator) MovieToNFO(movie *models.Movie, videoFilePath string) *Movie {
	return g.movieToNFO(context.Background(), movie, videoFilePath, nil)
}
...
func (g *Generator) Generate(ctx context.Context, movie *models.Movie, outputPath string, partSuffix string, videoFilePath string, tags []string) error {
	...
	tmplCtx.SetSourceFile(videoFilePath, "", "")
...
func (g *Generator) ResolveAndGenerate(ctx context.Context, movie *models.Movie, destDir string, nameCfg NFONameConfig, videoFilePath string, tags []string) (string, error) {
	...
	nfoFilename := ResolveNFOFilename(g.templateEngine, movie, nameCfg, videoFilePath)
...
func ResolveNFOFilename(engine template.EngineInterface, movie *models.Movie, cfg NFONameConfig, sourcePath ...string) string {
	if engine == nil {
		engine = template.NewEngine()
	}
	tmplCtx := template.NewContextFromMovieWithOptions(movie, template.ContextOptions{
	...
	if len(sourcePath) > 0 {
		tmplCtx.SetSourceFile(sourcePath[0], "", "")
	}
...
// renderDisplayTitle refreshes a display-title template when it depends on
// the actual video. Non-media templates keep the workflow-computed value.
func (g *Generator) renderDisplayTitle(movie *models.Movie, videoFilePath string, mediaInfo *mediainfo.VideoInfo) string {
	...
	if tmpl == "" || (fallback != "" && !templateNeedsVideoContext(tmpl)) {
		return fallback
	}
	...
	tmplCtx.SetSourceFile(videoFilePath, "", "")
	if mediaInfo != nil {
		tmplCtx.SetMediaInfo(mediaInfo)
	}

	rendered, err := g.templateEngine.Execute(tmpl, tmplCtx)
...
func templateNeedsVideoContext(tmpl string) bool {
	...
	return strings.Contains(upper, "<VR") ||
		...
		strings.Contains(upper, "<SOURCE") ||
		strings.Contains(upper, "<IF:SOURCE")
}
