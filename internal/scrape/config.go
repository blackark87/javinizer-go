...
func NewTranslatorFromApp(cfg *config.TranslationConfig) Translator {
	...
	ts := translation.New(bridgeCfg,
		...
		translation.NewBedrockProvider(bridgeCfg, httpClient),
	)
...
func (a *translationAdapter) TranslateTitles(ctx context.Context, titles []string) ([]string, error) {
	return a.svc.translateTitlesWithContext(ctx, titles)
}
