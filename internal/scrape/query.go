...
func (s *Scraper) queryAll(ctx context.Context, movieID, resolvedMovieID string, scrapers []models.Scraper, startTime time.Time) ([]*models.ScraperResult, []models.ScraperError) {
	...
	if s.cfg != nil && s.cfg.EarlyStop {
		return s.queryUntilCovered(ctx, resolvedMovieID, scrapers)
	}
...
func earlyStopMinimum(cfg *Config) int {
	if cfg == nil || cfg.EarlyStopMinResults < 1 {
		return 2
	}
	return cfg.EarlyStopMinResults
}

func scrapeResultsCoverRequiredFields(results []*models.ScraperResult, required []string) bool {
	for _, field := range required {
		...
		covered := false
		for _, result := range results {
			if result != nil && scraperResultCoversField(result, field) {
				covered = true
				break
			}
		}
...
func scraperResultCoversField(r *models.ScraperResult, field string) bool {
	switch field {
	case "id":
		return r.ID != ""
