...
func (s *Scraper) Scrape(ctx context.Context, cmd ScrapeCmd, progress ProgressFunc) (*ScrapeResult, error) {
	...
	skipCache := !cmd.CacheOnly && !cmd.RefreshTranslationOnly && (cmd.ForceRefresh || len(cmd.SelectedScrapers) > 0)
	...
	if len(cmd.SelectedScrapers) > 0 || resolverResult != nil {
		...
		scraped, aggResult, err = s.aggregator.AggregateWithPriority(aggregationResults, priority)
	} else {
		scraped, aggResult, err = s.aggregator.Aggregate(aggregationResults)
	}
	if err != nil {
		return nil, err
	}
