...
func ConfigFromApp(cfg config.TranslationConfig) Config {
	return Config{
		...
		TargetLanguages:         append([]string(nil), cfg.TargetLanguages...),
		...
		Bedrock:                 bedrockConfigFromApp(cfg.Bedrock),
	}
}

func bedrockConfigFromApp(cfg config.BedrockTranslationConfig) bedrockConfig {
