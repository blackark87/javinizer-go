...
var koreanTranslationReviewSourceTermRules = []koreanSourceTermRule{
	...
	{trigger: "育ち盛りHカップおっぱいちゃん", rule: "育ち盛りHカップおっぱいちゃん→한창 물오른 H컵 거유녀, 금지: 한창 자랄 나이의 H컵 가슴녀"},
	{trigger: "鬼エグストロングファッキン", rule: "鬼エグストロングファッキン→극강의 스트롱 퍽킹, 금지: 팍킹"},
	{trigger: "ずぽずぽ", rule: "sexual ずぽずぽ→푹푹 박다, 금지: 즈포즈포"},
	{trigger: "喘ぎ声", rule: "sexual 喘ぎ声→신음, 금지: 비명"},
	{trigger: "ギャラ飲み", rule: "ギャラ飲み→유료 술자리, 금지: 갤러로 술 마시기"},
	{trigger: "美女にチンポを挿入すると自ら腰を振り", rule: "美女にチンポを挿入すると自ら腰を振り→미녀에게 자지를 삽입하자 그녀가 스스로 허리를 흔들며; 행위 주체를 뒤집지 않는다"},
	{trigger: "マジ軟派、初撮。 1848", rule: "マジ軟派、初撮。 1848 뒤의 모든 제목 구절도 끝까지 번역하고 작품 번호에서 중단하지 않는다"},
	{trigger: "給料2ヶ月分の報酬に気持ちが揺らぎ", rule: "給料2ヶ月分の報酬に気持ちが揺らぎ→월급 2개월 치 보수에 마음이 흔들려, 금지: 흔동려"},
	{trigger: "ぐにぐにズボッ", rule: "ぐにぐにズボッ→말캉말캉, 쑥, 금지: 구니구니 즈봇"},
	{trigger: "布1ミリの壁を突破", rule: "布1ミリの壁を突破→천 1mm의 벽을 돌파, 금지: 돌방파"},
	{trigger: "義妹がロングスカート", rule: "義妹がロングスカート→의붓여동생이 롱스커트, 금지: 의붓여동엇생"},
	{trigger: "理性が崩壊する家庭教師のボク", rule: "理性が崩壊する家庭教師のボク→이성이 붕괴되는 과외 선생님인 나, 금지: 붕락"},
	...
	{trigger: "勃起チ○ポをいきなりアナルに突っ込まれ", rule: "침입 문맥 勃起チ○ポをいきなりアナルに突っ込まれ→침입자가 발기한 자지를 갑자기 애널에 쑤셔 넣자, 금지: 애널에 찔려"},
	{trigger: "厳選10人総集編付き2枚組", rule: "厳選10人総集編付き2枚組→엄선 10인 총집편 포함 2매 세트, 금지: 10 인원/포함2매"},
	{trigger: "2穴中出し集団痴●バス", rule: "2穴中出し集団痴●バス→2홀 질내사정 집단 치한 버스, 금지: 2두 구멍/치녀 버스"},
	{trigger: "痴●師", rule: "痴●師→치한, 금지: 치녀"},
	{trigger: "スローピストンの痺れる快感", rule: "スローピストンの痺れる快感→슬로우 피스톤의 저릿한 쾌감, 금지: 슬로 피스톤"},
	{trigger: "中出しも拒めない", rule: "中出しも拒めない→질내사정도 거부할 수 없다, 금지: 거무할 수 없다"},
	{trigger: "ドスケベボディギャル", rule: "ドスケベボディギャル→지독하게 야한 몸매의 갸루, 금지: 색녀 몸매"},
	{trigger: "少●たちを無差別に狙う", rule: "少●たちを無差別に狙う→어린 소녀들을 무차별적으로 노린, 목적어와 수식 방향을 보존"},
	...
	{trigger: "ネットでAV応募→AV体験撮影", rule: "ネットでAV応募→AV体験撮影→인터넷으로 AV 지원→AV 체험 촬영, preserve both stages, the arrow, and a trailing work number; never duplicate AV"},
	{trigger: "3P経験", rule: "3P経験→3P 경험, never drop P"},
	{trigger: "4パコ2日", rule: "4パコ2日→2일간 섹스 4회, 금지: 4파코2일/4섹스2일"},
	{trigger: "奇跡のカラダ", rule: "奇跡のカラダ→기적의 몸매, 금지: 기적의 몸"},
	{trigger: "性欲爆発", rule: "性欲爆発→성욕 폭발, 금지: 성욕 폭무"},
	...
	{trigger: "白目イキ怒涛絶頂3発射", rule: "白目イキ怒涛絶頂3発射→눈이 뒤집히며 노도 같은 절정, 3회 사정"},
	{trigger: "ダーツナンパ", rule: "ダーツナンパ→다트 헌팅, 금지: 다츠 난파/다트 난파"},
	{trigger: "喉奥イマラ", rule: "喉奥イマラ→목구멍 깊숙이 이라마치오, 금지: 목구멍 깊숙이 빨아대다"},
	{trigger: "暴発必至", rule: "暴発必至→참지 못하고 사정, 금지: 사정 폭발 필연"},
	...
	{trigger: "スケスケ＆ぱっくりエロ下着", rule: "スケスケ＆ぱっくりエロ下着→속이 비치고 보지가 훤히 드러나는 야한 속옷"},
	{trigger: "M男のち●こ全部ヌク大作戦", rule: "M男のち●こ全部ヌク大作戦→M남의 자지를 전부 빼주는 대작전, 금지: 대작략"},
	{trigger: "発禁", rule: "発禁→발매 금지, 금지: 발금/금지만 단독 사용"},
	{trigger: "睾丸パンパン＆ギン勃ち激アツ", rule: "睾丸パンパン＆ギン勃ち激アツ→고환 빵빵 & 빳빳하게 발기해 초대박, 금지: 핵뜨거움"},
	{trigger: "ローター", rule: "ローター→로터|진동 로터, 금지: 리모트 바이브/리모터"},
	{trigger: "ローターギチ詰めチャレンジ", rule: "ローターギチ詰めチャレンジ→로터 꽉 채우기 챌린지"},
	{trigger: "ギチ詰めローターチャレンジ", rule: "ギチ詰めローターチャレンジ→꽉 채우는 로터 챌린지"},
	...
	{trigger: "生ハメ", rule: "生ハメ→노콘; 生ハメSEX→노콘 섹스; 生ハメ中出し→노콘 질내사정, 금지: 생삽입/생하메"},
	{trigger: "潮吹き", rule: "sexual 潮吹き→분수|애액 분출, 금지: 시오후키/음차"},
	{trigger: "極上", rule: "praise 極上→최고|최상급, 금지: 극상"},
	{trigger: "エロい", rule: "ordinary adjective エロい→야한, 금지: 에로한"},
	{trigger: "エロフラグ", rule: "compound/series term エロフラグ→에로 플래그; shorter エロい→야한 rule does not apply inside it"},
	{trigger: "エロテク", rule: "compound エロテク→에로 테크닉; shorter エロい→야한 rule does not apply inside it"},
	{trigger: "素股", rule: "素股→가랑이딸, 금지: 스마타"},
	{trigger: "激クンニ", rule: "激クンニ→격렬한 보빨, 금지: 격렬한 쿤니"},
	{trigger: "デカチン", rule: "デカチン→대물, 금지: 대물 자지"},
	{trigger: "浴衣の上からでもわかる", rule: "浴衣の上からでもわかる→유카타 위로도 알 수 있는, 금지: 유무카타"},
	{trigger: "デカ尻に搾り取られ", rule: "デカ尻に搾り取られ→큰 엉덩이에 쥐어짜이며, 금지: 착취당하며"},
	{trigger: "第21弾", rule: "第21弾→제21탄, 금지: 제2릿탄"},
	{trigger: "ちびっ子水泳イントラ", rule: "성인 ちびっ子水泳イントラ→아담한 수영 강사, 금지: 꼬마 수영 강사"},
	{trigger: "言葉責め", rule: "sexual 言葉責め→음란어 조교|말로 괴롭히기, 금지: 말로 괴롭히는 섹스"},
	{trigger: "バブみ満点", rule: "バブみ満点→모성미 가득, 금지: 아기 취급"},
	{trigger: "童貞喪失3本立てSP", rule: "童貞喪失3本立てSP→동정 탈출 3편 구성 SP, 금지: 3 본집"},
	{trigger: "新雪の様な美肌", rule: "新雪の様な美肌→갓 내린 눈처럼 하얀 피부, 금지: 신설 같은 피부"},
	{trigger: "パジャマdeおじゃま", rule: "パジャマdeおじゃま→파자마 입고 실례할게요, 금지: 파자금"},
	{trigger: "即尺フェラ抜き", rule: "即尺フェラ抜き→바로 펠라로 빼주기, 이 구간에 삽입 행위를 추가하지 않는다"},
	{trigger: "【モ無】", rule: "FC2 tag 【モ無】→【모자이크 없음】, 금지: 【무모】/【모무】"},
	{trigger: "【無】", rule: "FC2 bracket tag 【無】→【무수정】, 금지: 【무】"},
	{trigger: "オナホみたいに**てたら", rule: "masked オナホみたいに**てたら→오나홀처럼 다루다 보니; 마스킹된 동사를 구체적 행위로 창작하지 않는다"},
	{trigger: "H柔爆乳", rule: "H柔爆乳→H컵 말랑 폭유, 금지: H야한 폭유"},
	{trigger: "ヤリマン男", rule: "ヤリマン男→섹스광 남자, 금지: 문란녀를 쫓는 남자"},
	{trigger: "お天気お姉さん", rule: "お天気お姉さん→기상캐스터, 금지: 날씨 여보단"},
	{trigger: "呻くように喘ぎだす", rule: "呻くように喘ぎだす→끙끙 앓듯 신음하기 시작한다, 금지: 신음하듯 애태우는"},
	{trigger: "必見", rule: "promotional 必見→놓치지 말 것|꼭 확인, 금지: 필시 확인"},
	{trigger: "18禁", rule: "18禁→18금, 금지: 18금지"},
	{trigger: "1人暮らし", rule: "1人暮らし→혼자 사는, 숫자 1을 별도로 출력하지 않는다"},
	{trigger: "輪●", rule: "sexual assault 輪●→윤간, 금지: 윤락"},
	{trigger: "威裸魔恥汚", rule: "stylized 威裸魔恥汚→이라마치오, 한자 뜻을 조합하지 않는다"},
	{trigger: "めるてぃー", rule: "performer nickname めるてぃー→메루티, 금지: 메루티 -/메루티ー"},
	{trigger: "串刺しJ○痴●", rule: "串刺しJ○痴●→꼬치처럼 꿰뚫리는 여고생 치한; 치한 2명의 수량은 한 번만 자연스러운 위치에 유지하고 치한범2처럼 붙이지 않는다"},
	{trigger: "温泉媚薬中出し痴●", rule: "温泉媚薬中出し痴●→온천 최음제 질내사정 치한, 금지: 치녀"},
	{trigger: "ナマ4発", rule: "ナマ4発→노콘 질내사정 4회, 금지: 노콘4 발"},
	{trigger: "エチエチSEX計3発射", rule: "エチエチSEX計3発射→야한 섹스, 총 3회 사정, 금지: SEX3 발사"},
	{trigger: "エステ狂い", rule: "エステ狂い→에스테틱 중독, 금지: 에스테틱 광적인 집착"},
	{trigger: "盗撮", rule: "盗撮→몰래 촬영|몰카, 금지: 도둑 촬영"},
	{trigger: "メンエス嬢", rule: "メンエス嬢→남성 전용 에스테틱 관리사, 금지: 남자 에스테틱 관리사"},
	{trigger: "競泳水着", rule: "競泳水着→경기용 수영복, 금지: 경영 수영복"},
	{trigger: "美翔女", rule: "美翔女→미소녀, 금지: 미쇼죠"},
	{trigger: "性癖射精ダービー", rule: "性癖射精ダービー→성적 취향별 사정 더비, 금지: 성벽 사정 더비"},
	{trigger: "カウントダウンパーティー", rule: "カウントダウンパーティー→카운트다운 파티, 금지: 카운트한 파티"},
	{trigger: "時の静寂", rule: "time-stop work 時の静寂→시간의 정지, 금지: 시간의 정적"},
	{trigger: "まいさん", rule: "performer まいさん→마이 씨, 금지: 마이짱"},
	{trigger: "超エロい", rule: "超エロい→엄청 야한, 금지: 초로로 야한"},
	...
	{trigger: "都月るいさ", rule: "performer 都月るいさ reading とつきるいさ→토츠키 루이사"},
	{trigger: "森沢かな", rule: "performer 森沢かな reading もりさわかな→모리사와 카나"},
	{trigger: "美甘りか", rule: "performer 美甘りか reading みあまりか→미아마 리카"},
	{trigger: "有賀みなほ", rule: "performer 有賀みなほ reading ありがみなほ→아리가 미나호"},
	{trigger: "新村あかり", rule: "performer 新村あかり reading にいむらあかり→니이무라 아카리"},
