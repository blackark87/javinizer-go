...
func koreanJAVPromptRules(targetLang string) string {
	...
	return "Korean JAV rules (non-negotiable priority): preserve the source's sexual explicitness exactly. Use direct current Korean AV vocabulary without sanitizing it, but never invent a sex act, insult, coercion, or stronger intensity absent from the source. Hard and explicit wording must still be fluent and cleanly written; it does not mean corny, food-like, or needlessly dirty phrasing. Prefer an established Korean AV term whenever one exists. Use a loanword only when no suitable Korean term exists or the source itself uses an established acronym such as POV, VR, or ASMR. " +
		...
		"逆レ and 逆レイプ are abbreviations for reverse rape and are not 逆パコ: use 역강간 or 여자가 강제로 덮치는 according to grammar. 逆レ搾精 means forced semen extraction by a woman: use 역강간 착정 or 여자가 강제로 덮쳐 정액을 뽑아내기, never invent or insert 역파코 or 파코 when パコ is absent from the source. " +
		...
		"The Latin title suffix chu is a cute kiss sound, not Japanese 中 and not the Korean progressive suffix 중. Preserve that kiss styling as 츄 (or use 쪽 only when required by sentence grammar); ガチ恋営業chu must end in 츄, never 중. " +
		...
		"When 沼 is a slang suffix for an irresistible fixation, express the attraction as 푹 빠지는 or 헤어나올 수 없는 according to grammar, never use the literal 늪. Thus ビッチ沼 means 문란녀에게 푹 빠지는 or 헤어나올 수 없는 문란녀의 매력. 都合のイイ describing a casual sexual partner means 원할 때 만날 수 있는 or 필요할 때 만나는, never the objectifying dictionary calque 편리한 and never invent 조건 좋은. In entertainment or idol context 枕営業 means 성상납, never leave it in Japanese and never blur it into the broader 스폰. 濃交 is promotional wordplay for intense intimate contact: use 농밀한 교감 or 진한 교감, never transliterate it as 농교. 色白 describes pale or fair skin: use 하얀 피부 or 피부가 하얀, never the dictionary calque 색백. 美巨乳 means 예쁜 거유 or 아름다운 거유, never the stacked calque 미거유. エロかわ and エロ可愛い mean 야하고 귀여운, never 에로 귀여운. In an erotic service context 乳首エステ means 유두 마사지, never 유두 에스테 and never broaden it to the vague 특별 코스. 舐めテク and ハンドテク mean 혀 테크닉 and 손 테크닉, never the stiff 핥기 기술 or 핸드 기술. 僕の身代わりに means 나 대신, never mechanically render の as possessive 내. バクヌキ means intensely or repeatedly getting someone off: use 실컷 빼주는 or 마구 뽑아주는 according to grammar, never transliterate it as 바쿠누키. 挟射 means ejaculation while held between the breasts: use 가슴 사이에 끼워 사정. In a sexually disparaging compound よわよわ means pathetic or hopeless rather than physically weak: おま○こよわよわ → 보지 허접, never 보지 약한. " +
