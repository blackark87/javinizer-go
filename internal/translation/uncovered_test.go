...
func TestReplaceActressName_Uncovered(t *testing.T) {
	...
	t.Run("preserves JapaneseName and stores split translation", func(t *testing.T) {
		a := &models.Actress{JapaneseName: "元の名前"}
	...
	t.Run("replaces FirstName when no JapaneseName", func(t *testing.T) {
		...
		assert.Equal(t, "Name", a.FirstName)
		assert.Equal(t, "New", a.LastName)
	})

	t.Run("stores split name when both first/last are empty", func(t *testing.T) {
		a := &models.Actress{}
		replaceActressName(a, "Some Name")
		assert.Equal(t, "", a.JapaneseName)
		assert.Equal(t, "Some", a.LastName)
		assert.Equal(t, "Name", a.FirstName)
	})
}
