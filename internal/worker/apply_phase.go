...
func (p *applyPhase) Run(ctx context.Context, inputs applyPhaseInputs, cfg ApplyPhaseConfig) {
	...
	skippedFiles := make([]string, 0, len(cfg.SkipFiles))
	for filePath, fileResult := range inputs.Results {
		eligibleStatus := fileResult.Status == models.JobStatusCompleted
		if cfg.Resume {
			eligibleStatus = eligibleStatus || fileResult.Status == models.JobStatusFailed || fileResult.Status == models.JobStatusCancelled
		}
		if !eligibleStatus || fileResult.Movie == nil {
			continue
		}
		...
		if cfg.Resume && cfg.SkipFiles[filePath] {
			logging.Infof("Skipping already-applied file during resume: %s", filePath)
	...
	total := len(items) + len(skippedFiles)
	var processed int64
	for _, filePath := range skippedFiles {
