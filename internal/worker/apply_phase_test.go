...
func TestApplyPhase_Run_ResumeSkipsCompletedAndRetriesFailed(t *testing.T) {
	wf := &stubApplyWorkflow{
		applyResult: &workflow.ApplyResult{Movie: &models.Movie{ID: "IPX-778"}},
	}
	inputs := makeApplyInputs(wf)
	inputs.Results["/source/already-done.mp4"] = &MovieResult{
		FileMatchInfo: models.FileMatchInfo{Path: "/source/already-done.mp4", MovieID: "IPX-777"},
		Status:        models.JobStatusCompleted,
		Movie:         &models.Movie{ID: "IPX-777"},
	}
	inputs.Results["/source/retry-me.mp4"] = &MovieResult{
		FileMatchInfo: models.FileMatchInfo{Path: "/source/retry-me.mp4", MovieID: "IPX-778"},
		Status:        models.JobStatusFailed,
		Movie:         &models.Movie{ID: "IPX-778"},
	}

	var organizedFiles []string
	...
	NewApplyPhase().Run(context.Background(), inputs, ApplyPhaseConfig{
		...
		Destination:     "/output",
		Resume:          true,
		SkipFiles:       map[string]bool{"/source/already-done.mp4": true},
		OnFileOrganized: func(filePath string) {
			organizedFiles = append(organizedFiles, filePath)
		},
		OnFileProgress: func(processed, total int) {
			progressCalls++
			assert.Equal(t, 2, total)
			assert.GreaterOrEqual(t, processed, 1)
			assert.LessOrEqual(t, processed, 2)
		},
		OnPhaseComplete: func(organized, failed int) {
			gotOrganized = organized
			gotFailed = failed
	...
	assert.Equal(t, 1, wf.getApplyCalled(), "only the failed file should be applied again")
	assert.ElementsMatch(t, []string{"/source/already-done.mp4", "/source/retry-me.mp4"}, organizedFiles)
	assert.Equal(t, 2, progressCalls)
	assert.Equal(t, 2, gotOrganized)
	assert.Zero(t, gotFailed)
