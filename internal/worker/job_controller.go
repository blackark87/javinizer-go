...
func (c *jobController) StartApply(ctx context.Context, cfg ApplyPhaseConfig) error {
	...
	c.job.lifecycle.setCancelFunc(cancel)
	expectedStatuses := []models.JobStatus{models.JobStatusCompleted}
	if cfg.Resume {
		expectedStatuses = append(expectedStatuses, models.JobStatusCancelled)
	}
	if err := c.markStartedFrom(expectedStatuses...); err != nil {
		cancel()
		return err
	}
...
func (c *jobController) Rescrape(ctx context.Context, cmd RescrapeCmd) (*RescrapeResult, error) {
	...
	if outcome.Status == models.RescrapeStatusSuccess && outcome.FilePath != "" &&
		(outcome.FieldSources != nil || outcome.ActressSources != nil || outcome.ScraperResults != nil || outcome.Candidates != nil || outcome.HasConflict) {
...
func (c *jobController) markStarted(expectedFrom models.JobStatus) error {
	return c.markStartedFrom(expectedFrom)
}

// markStartedFrom transitions the job to running when its current status is
// one of expectedFrom. Resume uses this compare-and-swap to safely restart a
// cancelled apply without weakening the normal Completed-only transition.
func (c *jobController) markStartedFrom(expectedFrom ...models.JobStatus) error {
	...
	if !allowed {
		...
		return fmt.Errorf("job %s: cannot start — expected status in %v but got %s", c.job.ID.String(), expectedFrom, actual)
	}
	...
	c.job.lifecycle.cancelled = false
	c.job.lifecycle.mu.Unlock()
