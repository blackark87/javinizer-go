...
func (o *applyOrchImpl) stepDisplayTitle(ctx context.Context, cmd ApplyCmd, state *applyPipelineState, steps *stepCompletion) error {
	if o.applyCfg.DisplayTitle != "" && o.templateEngine != nil {
		...
		ApplyDisplayTitleFromSourceFile(ctx, state.movie, titleSrc, o.applyCfg.DisplayTitle, o.templateEngine, o.applyCfg.NFONameCfg, videoPath)
...
func (o *applyOrchImpl) stepDownload(ctx context.Context, cmd ApplyCmd, state *applyPipelineState, steps *stepCompletion) error {
	...
	outcome, dlErr := o.downloader.Download(ctx, downloader.DownloadCmd{
		...
		SourcePath:          resolveApplyVideoPath(cmd.Match.Path, state.organizeResult),
		Multipart:           multipart,
...
func resolveApplyVideoPath(sourcePath string, organizeResult *organizer.OrganizeResult) string {
	if organizeResult != nil && organizeResult.NewPath != "" {
		return organizeResult.NewPath
	}
	return sourcePath
}
