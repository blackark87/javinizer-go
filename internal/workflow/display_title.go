...
func ApplyDisplayTitleFromSource(ctx context.Context, scraped *models.Movie, titleSource *models.Movie, displayTitleTmpl string, templateEngine template.EngineInterface, nameCfg nfo.NFONameConfig) {
	ApplyDisplayTitleFromSourceFile(ctx, scraped, titleSource, displayTitleTmpl, templateEngine, nameCfg, "")
}

// ApplyDisplayTitleFromSourceFile applies a DisplayTitle template with the
// source video attached, enabling source-path, resolution, and VR tags during
// the Apply phase while keeping scrape-only behavior unchanged.
func ApplyDisplayTitleFromSourceFile(ctx context.Context, scraped *models.Movie, titleSource *models.Movie, displayTitleTmpl string, templateEngine template.EngineInterface, nameCfg nfo.NFONameConfig, sourcePath string) {
	...
	if displayTitleTmpl != "" {
		...
		displayCtx.SetSourceFile(sourcePath, "", "")
