...
class APIClient {
	...
	async listActressMovies(id: number, limit?: number, offset?: number) {
		return this.actresses.listActressMovies(id, limit, offset);
	}
	...
	async bulkDeleteActresses(ids: number[]) {
		return this.actresses.bulkDeleteActresses(ids);
	}
	async deleteAllActresses() {
		return this.actresses.deleteAllActresses();
	}
