...
describe('organize-controller resume requests', () => {
	it('sets the resume flag so the backend skips completed file operations', async () => {
		const organizeBatchJob = vi.fn().mockResolvedValue(undefined);
		const { deps } = makeDeps({ organizeBatchJob });
		const controller = createOrganizeController(deps);

		await controller.resumeOrganize(true, false);

		expect(organizeBatchJob).toHaveBeenCalledWith(
			'job-1',
			expect.objectContaining({ resume: true, skip_nfo: true, skip_download: false }),
		);
		controller.cleanup();
	});
});
